...
#define SMAWS_DESC_STUB "%1 &#xA9;%2 %3 - generated by MuseScore %4 + SMAWS&#x2122; %5"
                            ...
                            QStringList*  pVTT           =  0)
{
    ...
    if (isMulti && idxStaff > -1 && pINames != 0  && pVisibleStaves != 0) {
        ...
        if (isStaff) {
            ...
            // Round these for crisp 1px horizontal lines with translate(0 0.5)
            top = qRound(top);
            bot = qRound(bot);
            ...
            printer->beginMultiGroup(pINames, pFullNames, className, height, top + 0.5,
...
static QString smawsDesc(Score* score) {
    return QString(SMAWS_DESC_STUB).arg(score->title())
                                   .arg(score->metaTag("copyright"))
                                   .arg(score->composer())
                                   .arg(VERSION)
                                   .arg(SMAWS_VERSION);
}
    ...
    foreach (const Element* e, elmPtrs) {
        ...
        if (isMulti && idxStaff != idx) {
            ...
            paintStaffLines(score, &p, &printer, page, &visibleStaves, nVisible,
                            idx, isMulti, &iNames, &fullNames, &staffTops, &setVTT);
            idxStaff = idx;
        }
...
///!!!OBSOLETE!!!        printer.setStartMSecs(startMSecsFromCueID(score, cue_id));
    ...
    if (isMulti) {
        ...
        printer.beginMultiGroup(&iNames, 0, system, 35, 0, QString()); ///!!! 35 is standard top staff line y-coord, I'm being lazy here by hardcoding it
        bool isMouse = false;
