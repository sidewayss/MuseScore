                            ...
                            int           lyricsHeight = -1)
{
    ...
    if (isMulti) {
        ...
        if (mapLyrics->size() > 0) {
            printer->beginMultiGroup(0, 0, CLASS_LYRICS,
                                     lyricsHeight,
                                ...
                                if ((*pageNames[idxPage])[r] == 0) {
                                    if (idxPage == 0)
                                        (*pageNames[idxPage])[r] = new QString(
                                            stringToUTF8(score->staff(r)->part()->longName(startOffset), true)
                                                                              );
                                    else
        ...
        << SVG_4SPACES << SVG_POINTER   << SVG_NONE      << SVG_QUOTE
                ...
                for (int r = 0; r < nStaves; r++) {
                    ...
                    if (r != idxGrid && iNames[r] != 0) {
                        ...
                        // long name for tooltip via <title>
                        title = stringToUTF8(score->staff(r)->part()->shortName(startOffset), true);
