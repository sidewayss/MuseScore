...
#define FILE_DRUM_DEFS  "SMAWS_DrumDefs.svg.txt"      // Drum Machine <defs>
...
#define FILE_FRET_DEFS  "SMAWS_FretDefs.svg.txt"      // Fretboard <defs>
...
#define FILTER_SMAWS_FRETS         "SMAWS Fretboard VTT"
...
///////////////////////////////////////////////////////////////////////////////
// saveVTT()
// Private, static function called by saveSMAWS_Lyrics() and saveSMAWS_Fretboard()
                         ...
                         const QString& indent = "")
{
    ...
    dataStart = QString::number(startMSecsFromCueID(score, cue_id));
    QFile       qf;
    QTextStream qts;
    QTextStream qtsFile;
    QString     qs;
    ...
    qts << qtsFile.readAll().replace("%10", QString::number(cntrWidth + 1))
                            ...
                            .replace("%14", dataStart)
                            .replace("%15", cue_id)
                            .replace("%16", evtPrefix)
                            .replace("%17", QString::number(cntrWidth + margin + 1));
    ...
    *streamMarks << QString(qs).arg(halfHeight - 1).arg(halfHeight - 1).arg(halfHeight - 1)
    ...
    const QString loopEnd   = QString("<polygon class=\"cursLoopLo\" points=\"-6,19 6,19 0,2\" transform=\"translate(%1.5,0)\" data-start=\"%2\" onmousedown=\"%3cursorDown(evt)\" onmouseup=\"%4cursorUp(evt)\"/>\n").arg(width - margin).arg(dataStart).arg(evtPrefix).arg(evtPrefix);
    ...
    for (QMap<QString, const Element*>::iterator i  = mapRulersSVG.begin();
        ...
        // is excluded, because it's not clickable. These rects must be after
...
bool MuseScore::saveSMAWS_Rulers(Score* score, QFileInfo* qfi)
{
    ...
    streamRulers(score, qfi, &fileStream, &markStream, wRuler - wButts, evtPrefix, SVG_4SPACES);
        ...
        if (m->repeatEnd()) {                                                  /// TABLE END ///
            ...
            if (isHTML) {
                ...
                if (hasRulers) {
                    ...
                    streamRulers(score, qfi, &barStream, &tableStream, width, evtPrefix, SVG_4SPACES);
...
bool MuseScore::saveSMAWS_Fretboard(Score* score, QFileInfo* qfi)
{
    Staff*      staff;
    Measure*    m;
    Segment*    s;
    ...
    QString fileName;
    QTextStream file;
    const QString fnRoot = QString("%1_Frets").arg(qfi->filePath().left(qfi->filePath().size() - 4));
...
bool MuseScore::saveSMAWS_Lyrics(Score* score, QFileInfo* qfi)
{
    ...
    const QString fnRoot   = QString("%1/%2").arg(qfi->path()).arg(score->metaTag("workNumber"));
