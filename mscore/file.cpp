...
#define SMAWS_RULERS "R"
#define SMAWS_TREE   "T"
#define SMAWS_SCORE  "S"
#define SMAWS_GRID   "G"
#define SMAWS_FRETS  "F"
#define SMAWS_PART   "P"
#define SMAWS_LYRICS "L"
#define SMAWS_VIDEO  "V"
...
#define FILE_RULER_G    "templates/SMAWS_RulerG.svg.txt"     // ends one <g> starts another with a translate
#define FILE_RULER_L    "templates/SMAWS_RulerLine.svg.txt"  // no events <use> element for ruler lines
...
#define SMAWS_DESC_STUB "copyright %1 %2 - generated by MuseScore %3 + SMAWS %4"
...
static int tickToMsec(Score* score, int tick)
{
...
static QString getCueID(Score* score, int startTick, int endTick = -1)
{
    // For cue_id formatting: 1234567_1234567
    const int   base     = 10;
    ...
    const int startMsec = tickToMsec(score, startTick);
    const int   endMsec = tickToMsec(score,   endTick);
    return QString("%1%2%3").arg(startMsec, CUE_ID_FIELD_WIDTH, base, fillChar)
                            .arg(SMAWS_)
                            .arg(endMsec,   CUE_ID_FIELD_WIDTH, base, fillChar);
}

static QString getCueID2(Score* score, int startTick, int endTick = -1)
{
    const int startMsec = tickToMsec(score, startTick);
    if (endTick < 0)
        return QString::number(startMsec);
    else {
        const int   base     = 10;
        const int   endMsec  = tickToMsec(score,   endTick);
        const QChar fillChar = SVG_ZERO;
...
static QString getAnnCueID(Score* score, const Element* e, EType eType)
{
    ...
    for (Segment* seg = segStart->next1MM(SegmentType::ChordRest);
        ...
        for (Element* eAnn : seg->annotations()) {
            if (eAnn->type() == eType)
                return getCueID(score, startTick, seg->tick().ticks()); // USED BY GRID...
        }
    }

    // If there's no "next" annotation, this is the last one in the score
    return getCueID(score, startTick, score->lastSegment()->tick().ticks());
}
// getScrollCueID()
...
static int getScrollMsec(Score* score, const Element* e)
{
    int   msec;
    EType eType = e->type();
    Element* p  = e->parent();
    ...
    case EType::BAR_LINE       :
        ...
        case EType::SYSTEM  :
            ...
            msec = tickToMsec(score, static_cast<System*>(p)->firstMeasure()->tick().ticks());
            break;
        case EType::SEGMENT :
    ...
    case EType::KEYSIG     :
    case EType::TIMESIG    :
        msec = tickToMsec(score, static_cast<Segment*>(p)->tick().ticks());
        break;
...
static QString getScrollCueID(Score* score, const Element* e)
{
    ...
    case EType::REHEARSAL_MARK :
        cue_id = getAnnCueID(score, e, eType); // it has a duration
        break;
    case EType::BAR_LINE       :
    case EType::TEMPO_TEXT :
    case EType::CLEF       :
    ...
    case EType::BRACKET :
    case EType::INSTRUMENT_CHANGE :
        cue_id = QString::number(getScrollMsec(score, e));
        break;
                            ...
                            QList<qreal>* pStaffTops     =  0)
{
    ...
    if (isMulti && idxStaff > -1 && pINames != 0  && pVisibleStaves != 0) {
        ...
        if (isStaff) {
            ...
            const int slashHeight = 30 * DPI_F;
            ...
            // Standard notation, tablature, or slashes? I need to know by staff.
            ...
            const bool    isSlash   = (shortName == STAFF_SLASH);
            const int     height    = (isSlash ? slashHeight : bot - top);
            const QString className = (isSlash ? STAFF_SLASH
                                               : (isTab ? CLASS_TABS
                                                        : CLASS_NOTES));
            if (isTab && isLinked)
                qs.append("Tabs");
                            ...
                            int           lyricsHeight = -1)
{
    ...
    if (barLines != 0 && idx < 1) {
        for (auto c = barLines->begin(); c != barLines->end(); ++c) {
            printer->setCueID(QString::number(c.key()));
    ...
    if (mapFrozen->size() > 0) {
        ...
        IntList keys = mapFrozen->uniqueKeys();
        for (auto c = keys.begin(); c != keys.end(); ++c) {
            printer->setCueID(QString::number(*c));
    ...
    for (auto i = mapSVG->begin(); i != mapSVG->end(); ++i) {
    ...
    if (isMulti) {
        ...
        if (mapLyrics->size() > 0) {
            ...
            for (auto j = mapLyrics->begin(); j != mapLyrics->end(); ++j) {
...
// saveVTT()
// Private, static function called by saveSMAWS_Music()
...
//    Beam* b;
    foreach (const Element* e, elmPtrs) {
        ...
        case EType::TEMPO_TEXT        : /// Frozen Pane elements, all zero-duration
        case EType::INSTRUMENT_NAME   :
        case EType::INSTRUMENT_CHANGE :
        case EType::CLEF    :
        case EType::KEYSIG  :
...
//!!C++17            [[fallthrough]]; // fall-through is by design, no break here
        ...
        printer.setCueID(cue_id); // cue_id is always empty string here...
    ...
    if (isMulti) {
        ...
        printer.setCueID("");
        for (auto it : sysElms) {
              printer.setElement(it);
              paintElement(p, it);
        }
        for (auto it = mapSysFreeze.begin(); it != mapSysFreeze.end(); ++it) {
            elm = it.value();
            printer.setCueID(it.key());
            printer.setElement(elm);
            paintElement(p, elm);
        }
        for (auto it = mapSysCues.begin(); it != mapSysCues.end(); ++it) {
            cue_id = it.key();
            elm    = it.value();
                         ...
                         int            width)
{
    ...
    QString rects;    // markers <use>s must follow bars <text>s so that
    QString uses;     // they are on top (streamed after, separately).
    QString texts;
    QString line;
    ...
    QString   str;
    QString* pstr;
    QTextStream qtsRects(&rects);
    QTextStream qtsUses (&uses );
    QTextStream qtsTexts(&texts);
    ...
    Element*  e;
    EType     eType;
    int rectMsec; // bars ruler rects apply to the previous bar
    int prevMsec;
    ...
    const int cueIdDigits = QString::number(lastTick).size();
    ...
    for (m = score->firstMeasure(); m; m = m->nextMeasureMM()) {
        ...
        for (s = m->first(SegmentType::ChordRest); s; s = s->next(SegmentType::ChordRest)) {
            ...
            for (Element* eAnn : s->annotations()) {
                ...
                case EType::REHEARSAL_MARK :
                case EType::TEMPO_TEXT     :
                    msec = tickToMsec(score, s->tick().ticks());
                    mapSVG.insert(msec, eAnn);
        ...
        msec = tickToMsec(score, m->tick().ticks());
        mapSVG.insert(msec, static_cast<Element*>(m));
        setVTT->insert(msec);
    }
    ...
    getRulersTemplate(&texts, FILE_RULER_G,  qfi);
    getRulersTemplate(&line,  FILE_RULER_L,  qfi);
    ...
    *qts << qtsFile.readAll().arg(width)
    ...
    for (i = mapSVG.begin(); i != mapSVG.end(); ++i) {
        ...
        // Values for this cue
        msec = i.key(); // lines fall on whole number x-coordinates
        pxX  = qRound(margin + (pxPerMSec * msec));
        ...
        if (isTempo)
            qtsTempo << SVG_4SPACES << SVG_GROUP_BEGIN
                     << formatInt(SVG_CUE_NQ, msec, cueIdDigits, true)
                     << " data-tempo=\"" << static_cast<TempoText*>(e)->tempo()
                     << SVG_QUOTE << SVG_GT << SVG_GROUP_END << endl;
        else {
            // Invisible bar <rect>s and markers <use>s:
            ...
            if (msec > 0 || isMarker) {
                if (isMarker) {
                    qtsTmp.setString(&uses);
                    pstr = &rectM;
                    str  = (setVTT->find(msec) == setVTT->end()) 
                         ? " data-mid-bar=\"1\"" : "";
                }
                else {
                    rectX += rectWidth;
                    qtsTmp.setString(&rects);
                    pstr = &rectB;
                    str  = QString::number(rectWidth, 'f', 1);
                }
                qtsTmp << (*pstr)
                          .arg(formatInt (SVG_CUE_NQ, rectMsec, cueIdDigits, true))
                          .arg(str);
            }

            // Lines: both Markers and Bars get a line and, conditionally, text.
            qtsNoEvt << line.arg(formatInt(SVG_CUE_NQ, msec, cueIdDigits, true))
                            .arg(lineID);

            // Text: only stream the text element if there's text inside it
            if (!label.isEmpty()) 
                (isMarker ? qtsTexts : qtsNoEvt) 
                    << (isMarker ? textM : textB)
                        .arg(formatInt(SVG_CUE_NQ, msec, cueIdDigits, true))
                        .arg(label);
    ...
    qtsRects << rectB.arg(formatInt(SVG_CUE_NQ, msec, cueIdDigits, true));

    // Invisible <rect> for final end-of-bar line
    msec      = tickToMsec(score, lastTick);
    rectX    += rectWidth;
    rectWidth = width - rectX - 1;
    qtsRects << rectB.arg(formatInt(SVG_CUE_NQ, msec, cueIdDigits, true));

    // The final end-of-bar lines: top and bottom, aka markers and bars
    lineID = (++iBarNo % 5 == 0 ? line5 : line1);
    QString argCue = formatInt(SVG_CUE_NQ, msec, cueIdDigits, true);
    qtsNoEvt << line.arg(argCue).arg(lineID)
             << line.arg(argCue).arg(lineMrks);

    // Stream things in order and in groups as required
    *qts << rects           
         << SVG_4SPACES     << SVG_GROUP_END << endl << SVG_4SPACES   
         << SVG_GROUP_BEGIN << SVG_GT        << endl 
         << uses << texts 
         << SVG_4SPACES     << SVG_GROUP_END << endl 
         << SVG_2SPACES     << SVG_GROUP_END << endl << SVG_2SPACES
         << SVG_GROUP_BEGIN << SVG_GT        << endl 
         << noEvents    
         << SVG_2SPACES     << SVG_GROUP_END << endl << SVG_2SPACES
         << SVG_GROUP_BEGIN << SVG_GT        << endl
         << tempos
         << SVG_2SPACES     << SVG_GROUP_END << endl;
...
bool MuseScore::saveSMAWS_Rulers(Score* score, QFileInfo* qfi)
{
    ...
    fileStream << qts.readAll().arg(wRuler)
                               .arg(score->firstMeasure()->no() + 1)
                               .arg(score->metaTag("workTitle"))
                               .arg(smawsDesc(score));
                                 ...
                                 bool       hasRulers)
{
    ...
    int endTick     = 0; // this chord's     end tick, a temp variable
    ...
    QStringList    gridStart; //  ditto
    QStringList    gridEnd;   //  ditto
    StrPtrVectVect grid     (nGridCols, 0);  // by col, by row
    StrPtrVectVect dataStart(nGridCols, 0);  
    StrPtrVectVect dataEnd  (nGridCols, 0);  
    Int2IntMap     subCols;
    IntSet         startTimes;
    IntSet         endTimes;
    Int2IntSet     start2end;
    ...
    const int maxDigits  =   4; // for x/y value formatting, range = 0-9999px ///!!!should be dynamic!!!!
    ...
    for (m = score->firstMeasure(); m; m = m->nextMeasureMM())
    {
        ...
        for (s = m->first(SegmentType::ChordRest); s; s = s->next(SegmentType::ChordRest))
        {
            ...
            if (isGridCol) {                                                   /// GRID CHORD ///
                ...
                || (isPages && pageTick == mStartTick)) // or new page
                {
                    if (gridTicks == 0) {                                      /// NEW TABLE ///
                        ...
                        initialBPM    = prevTempo * BPS_2_BPM;
                        prevTempoPage = 0;

                        m1 = m; // this is useful later
                    }

                    // Deal with multiple pages, or not...
                ...
                if (spv == 0) {
                    ...
                    if (isPages) {
                        ...
                        if (is2) {
                            dataStart2[idxCol] = new StrPtrVect(nStaves, 0);
                            dataEnd2  [idxCol] = new StrPtrVect(nStaves, 0);
            ...
            startMsec = tickToMsec(score, startTick);
            startTimes.insert(startMsec);
            ...
            for (int r = 0; r < nStaves; r++) {
                ...
                if (r == idxGrid) {                                            /// GRID CELL ///
                    ...
                    endTick = startTick + gridTicks;
                    endMsec = tickToMsec(score, endTick);
                    endTimes.insert(endMsec);
                    start2end.emplace(startMsec, IntSet());
                    start2end[startMsec].insert(endMsec);

                    QString ref = STAFF_GRID;
                    ref += (isPages && idxPage > 0 ? LO: NO);
                    msec = tickToMsec(score, startTick - pageOffset);
                    // grid row <use> element
                    if (!isPages || idxCol == gridUse.size()) {
                        qts << SVG_USE << SVG_SPACE
                            << formatInt(SVG_X, cellX, maxDigits, true) // y ="0"
                            << XLINK_HREF << ref << SVG_QUOTE
                            << formatInt(SVG_COL_CUE, msec, cueIdDigits, true)
                            << SVG_CUE;
                        if (isPages) {
                            gridUse.append(pqs);
                            gridStart.append(QString::number(startMsec));
                            gridEnd  .append(QString::number(endMsec));
                        ...
                        gridStart[idxCol] += SVG_COMMA % QString::number(startMsec);
                        gridEnd  [idxCol] += SVG_COMMA % QString::number(endMsec);
                    }
                    ...
                    if (prevTempo != tempoMap->tempo(gridTick)) {
                        ...
                        if (tempoCues.isEmpty()) { // Is this the first tempo change?
                            qts << initialBPM;
                            prevTempoPage = 1;
                        }
                        while (idxPage > prevTempoPage++)
                            qts << SVG_COMMA; // any missing page cues

                        // The tempo change's cue
                        prevTempo = tempoMap->tempo(gridTick);
                        qts << SVG_COMMA << prevTempo * BPS_2_BPM;
                    }
                    ...
                    if (tick && !(tick % denom)) {
                        if (!isPages || idxBeat == beatLines.size()) {
                            ...
                            int h = cellHeight + gridMargin;
                            pqs = new QString;
                            qts.setString(pqs);
                            qts << SVG_RECT               // 1px width rect, not a line
                                   << formatInt(SVG_X, x, maxDigits, true)
                                   << formatInt(SVG_Y, h, 2, true)
                                   << SVG_WIDTH  << SVG_ONE << SVG_QUOTE
                                   << SVG_HEIGHT << height - (h * 2) << SVG_QUOTE
                    ...
                    if (!isGridCol) {                                          /// TUPLET or other non-grid sub-division ///
                        ...
                        if (subCols.find(startTick) == subCols.end()) {
                            ...
                            if (isPages) {
                                dataStart.append(new StrPtrVect(nStaves, 0));
                                dataEnd  .append(new StrPtrVect(nStaves, 0));
                                leds.append(new StrPtrListVect(nStaves, 0));
                                if (is2) {
                                    dataStart2.append(new StrPtrVect(nStaves, 0));
                                    dataEnd2  .append(new StrPtrVect(nStaves, 0));
                    ...
                    endTick = startTick + dataTicks;
                    endMsec = tickToMsec(score, endTick);
                    endTimes.insert(endMsec);
                    start2end[startMsec].insert(endMsec);
                    if (isChord2) {
                        endTick2 = startTick + dataTicks2;
                        endMsec2 = tickToMsec(score, endTick2);
                        endTimes.insert(endMsec2);
                        start2end[startMsec].insert(endMsec2);
                    }
                    ...
                    if (pqs->isEmpty()) {
                        if (isLED) { // LED staff, notes not text
                            qts << SVG_USE << SVG_SPACE
                                ...
                                << formatInt(SVG_COL_CUE, msec, cueIdDigits, true)
                                << HREF_EMPTY; // attribute required, even if no value!!!
                        }
                        else if (isChord) {
                    ...
                    if (isPages) {
                        ...
                        if (isChord) {
                            ...
                            if ((*dataStart[idxCol])[r] == 0) {
                                (*dataStart[idxCol])[r]  = new QString();
                                (*dataEnd  [idxCol])[r]  = new QString();
                               *(*dataStart[idxCol])[r] += QString::number(startMsec);
                               *(*dataEnd  [idxCol])[r] += QString::number(endMsec);
                            }
                            else {
                               *(*dataStart[idxCol])[r] += SVG_COMMA % QString::number(startMsec);
                               *(*dataEnd  [idxCol])[r] += SVG_COMMA % QString::number(endMsec);
                            }
                        }
                        ...
                        if (isChord2) {
                            // This cell's data-cue string
                            if ((*dataStart2[idxCol])[r] == 0) {
                                (*dataStart2[idxCol])[r]  = new QString(QString::number(startMsec));
                                (*dataEnd2  [idxCol])[r]  = new QString(QString::number(endMsec2));
                            }
                            else {
                               *(*dataStart2[idxCol])[r] += SVG_COMMA % QString::number(startMsec);
                               *(*dataEnd2  [idxCol])[r] += SVG_COMMA % QString::number(endMsec);
                            }
        ...
        if (m->repeatEnd()) {                                                  /// TABLE END ///
            ...
            if (isPages){
                ...
                qts << SVG_8SPACES << SVG_8SPACES;

                for (auto pageID = pageIDs.begin(); pageID != pageIDs.end(); ++pageID)
                    qts << (pageID == pageIDs.begin() ? SVG_L_BRACKET : SVG_COMMA)
                        << *pageID;
                ...
                if (tempoCues.isEmpty()) 
                    qts << initialBPM << SVG_QUOTE;
                else {
                    if (idxPage > prevTempoPage) {
                        qts << SVG_COMMA << prevTempo * BPS_2_BPM;
            ...
            tableStream
                << SVG_BEGIN
                   ...
                   << SVG_VIEW_BOX  << SVG_ZERO  << SVG_SPACE << SVG_ZERO << SVG_SPACE
                                    << width     << SVG_SPACE << height   << SVG_QUOTE
                   << SVG_HEIGHT    << height    << SVG_QUOTE
                   << SVG_POINTER   << SVG_NONE  << SVG_QUOTE << endl;

            if (isPages) {
                tableStream << SVG_4SPACES << SVG_CUE << SVG_L_BRACKET;
                for (auto sT = startTimes.begin(); sT != startTimes.end(); ++sT)
                    tableStream  << (sT == startTimes.begin() ? SVG_L_BRACKET : SVG_COMMA)
                                 << *sT;
                tableStream << SVG_R_BRACKET << SVG_COMMA << endl << SVG_8SPACES << SVG_8SPACES;
                for (auto eT = endTimes.begin(); eT != endTimes.end(); ++eT)
                    tableStream  << (eT == endTimes.begin() ? SVG_L_BRACKET : SVG_COMMA)
                                 << *eT;
...
//                tableStream << SVG_R_BRACKET << SVG_COMMA << endl << SVG_8SPACES << SVG_8SPACES;
//                for (auto s2e = start2end.begin(); s2e != start2end.end(); ++s2e) {
//                    tableStream  << (s2e == start2end.begin() ? SVG_L_BRACE : SVG_COMMA)
//                                 << SVG_QUOTE << s2e->first << SVG_QUOTE << SVG_COLON;
//                    for (auto eT = s2e->second.begin(); eT != s2e->second.end(); ++eT)
//                        tableStream  << (eT == s2e->second.begin() ? SVG_L_BRACKET : SVG_COMMA)
//                                     << *eT;
//                    tableStream << SVG_R_BRACKET;
//                }
//                tableStream << SVG_R_BRACE << SVG_COMMA << endl
//                            << pageCues    << SVG_R_BRACKET << SVG_SINGLE << endl
//                            << SVG_4SPACES << SVG_PAGE_CUE  << tempoCues  << SVG_GT << endl;
            }
            tableStream << SVG_DESC_BEGIN << score->metaTag("workTitle")
                        << SVG_SPACE      << smawsDesc(score) 
                        << SVG_DESC_END   << endl;
            ...
            if (isPages) {
                ...
                for (int b = 0; b < barLines.size(); b++) {
                    ...
                    hasPageCues = false;
                    barX = pageBars[b];
                    if (barX->size() < idxPage)     // missing pages
                        hasPageCues = true;
                    else {
                        const int x = (*barX)[0];   // initial x
                        for (int p = 1; p < idxPage; p++) {
                            if ((*barX)[p] != x) {  // diff x-coords
                                hasPageCues = true;
                                break;
                            }
                        }
                    ...
                    if (hasPageCues) { // page cues required for barNums[b]
                        for (int p = 0; p < idxPage; p++) {
                            ...
                            if (p < pageBarNums[b]->size() && (*pageBarNums[b])[p] > offScreen)
                                qts << (*pageBarNums[b])[p];
                        }
                        qts << SVG_QUOTE;
                    }
                    qts << SVG_GT << SVG_TEXT_END << endl;
                }
            }
            ...
            for (int b = 0; b < beatLines.size(); b++) {
                ...
                if (idxPage > 1) { // are there page cues for this beatline?
                    ...
                    if (hasPageCues) { // page cues required
                        tableStream << SVG_PAGE_CUE;
                        for (int p = 0; p < idxPage; p++) {
                            if (p > 0)
                                tableStream << SVG_COMMA;
                            beatX = p < pageBeats[b]->size() ? (*pageBeats[b])[p]
                                                             : offScreen;
                            if (beatX != prevBeatX)
                                tableStream << beatX;
            ...
            tableStream << SVG_GROUP_END   << endl
                        << SVG_GROUP_BEGIN << SVG_POINTER << SVG_VISIBLE
            ...
            if (isPages) {
                ...
                // Stream the <use> and <text> into separate <g>s
                tableStream << SVG_2SPACES << SVG_GROUP_BEGIN << SVG_GT << endl;
                for (int g = 0; g < nGridCols; g++) {
                    tableStream << SVG_4SPACES  << *gridUse[g]   << SVG_L_BRACKET << SVG_L_BRACKET
                                << gridStart[g] << SVG_R_BRACKET << SVG_COMMA     << SVG_L_BRACKET 
                                << gridEnd[g]   << SVG_R_BRACKET << SVG_R_BRACKET << SVG_QUOTE;
                    if (g >= minCols) { // grayed-out <use>s by page
                        ...
                        for (int p = 0; p < idxPage; p++) {
                            ...
                            if (g < pageCols[p]) {
                                if (prevVal) 
                                    tableStream << SVG_SPACE;
                                else 
                                    tableStream << CLASS_GRID << NO;
                                prevVal = true;
                            }
                            else
                                prevVal = false;
            ...
            for (int r = 0; r < nStaves; r++) {         /// for each row
                ...
                const bool isChordsRow = (r == idxChords);
                const bool hasPitches  = (pitchSet[r] != 0 && pitchSet[r]->size() > 1);
                QStringList names;
                ...
                if (isPages && !isGridRow) {
                    tableStream << SVG_2SPACES << SVG_GROUP_BEGIN << SVG_CLASS 
                                << CLASS_NOTES << SMAWS_GRID      << SVG_QUOTE;
                ...
                if (!(isPages && isGridRow)) {
                    for (int c = 0; c < grid.size(); c++) { /// for each column
                        ...
                        if (isPages) {                        // ¿Page Cues?
                            ...
                            hasPageCues = (changesPitch || changesValue);
                            if (hasPageCues) { // Page Cues
                                ...
                                double pitchY;
                                int pitch;
                                int prevPitch = -99; // pitch can be -1 or -2
                                qts.setString(&cueY);
                                for (int p = 0; p < idxPage; p++) {
                                    if (changesValue) {
                                        tableStream << (p == 0 ? SVG_PAGE_CUE : QString(SVG_COMMA));
                                        if (spl->size() > p) {
                                            led = (*spl)[p];
                                            if (!led->isEmpty()) {
                                                if (hasPitches)
                                                    led->replace(LED, MINI);
                                                if (prevLed != 0 && *led == *prevLed)
                                                    tableStream << SVG_SPACE;
                                                else {
                                                    tableStream << *led;
                                                }
                                    ...
                                    if (changesPitch) {
                                        qts << (p == 0 ? " data-y=\"" : QString(SVG_COMMA));
                                        if (pil->size() > p) {
                                            pitch = (*pil)[p];
                                            if (pitch != MIDI_EMPTY) {
                                                if (pitch == prevPitch)
                                                    ...
                                                    if (pitch == MIDI_REST)
                                                        qts << (hasPitches ? restOffset : 0);
                                                    else if (pitch != MIDI_EMPTY) {
                                                        pitchY = intervals[r] * pitchSet[r]->indexOf(pitch);
                                                        if (pitchY == 0)
                                                            qts << SVG_ZERO;
                                                        else
                                                            qts << QString::number(pitchY, 'f', SVG_PRECISION);
                                                    }
                                                }
                                            }
                                            prevPitch = pitch;
                        ...
                        if (isPages) {                      // page      cues
                            if ((*dataStart2[c])[r] != 0) { // highlight cues
                                tableStream << SVG_CUE << SVG_L_BRACKET << SVG_L_BRACKET
                                            << *(*dataStart2[c])[r] << SVG_R_BRACKET << SVG_COMMA << SVG_L_BRACKET
                                            << *(*dataEnd2  [c])[r] << SVG_R_BRACKET << SVG_R_BRACKET << SVG_QUOTE;

                                spl = (*leds2[c])[r];
                                if (spl != 0) {            ///!!!lyrics row needs testing!!!
                                    for (int p = 0; p < idxPage; p++) {
                                        if (p == 0)
                                            tableStream << SVG_PAGE_CUE;
                                        else
            ...
            const int x  = (isPages && idxPage > 0 ? offScreen : cX - barRound);
            if (!isPages || (idxBar == barLines.size() && m != mPageEnd)) {
                ...
                qts.setString(pqs);
                qts << SVG_TEXT_BEGIN
                       << formatInt(SVG_X, x + 4, maxDigits, true)
                       << formatInt(SVG_Y, barMargin, 2, true)
                       << SVG_CLASS << "barNumber" << SVG_QUOTE;
                if (!isPages)
            ...
            if (isPages) {
                ...
                while (pageBars[idxBar]->size() < idxPage) {
                    pageBars[idxBar]->append(offScreen);    // This barline is initially invisible
                    pageBarNums[idxBar]->append(offScreen); // It's bar number reflects that
                }
...
bool MuseScore::saveSMAWS_Tree(Score* score, QFileInfo* qfi)
{
             ...
             t += VOICES)
    {
                      ...
                      m = m->nextMeasureMM())
        {
            ...
            if (m->isEmpty(idx)) {
                ...
                if (!isPulse && !isPrevRest) {
                    isPrevRest = true;
                    mapMix.insert(getCueID(score, startTick, m->tick().ticks()), iName);
                }

                // Empty measure following a non-empty measure. Start of
                // a possibly multi-measure rest.
                if (pm != 0 && !pm->isEmpty(idx))
                    startTick = m->tick().ticks();

                // Final measure is empty
                if (!m->nextMeasureMM())
                    mapMix.insert(getCueID(score, startTick, m->tick().ticks() + m->ticks().ticks()), notName);
            }
            // Non-empty measures have highlight and pulse cues
            else {
                // Complete any pending gray-out cue
                if (pm != 0 && pm->isEmpty(idx))
                    mapMix.insert(getCueID(score, startTick, m->tick().ticks()), notName);
                              ...
                              s = s->next(SegmentType::ChordRest))
                {
                    ...
                    case EType::CHORD :
                        ...
                        if (isPulse) {
                            ...
                            mapMix.insert(getCueID(score, startTick, cr->tick().ticks() + cr->actualTicks().ticks()), 
                                          iName);
                    ...
                    case EType::REST :
                        if (!isPulse && !isPrevRest) {
                            isPrevRest = true;
                            mapMix.insert(getCueID(score, startTick, cr->tick().ticks()), iName);
                        }
                        break;
                    default:
        ...
        if (!isPulse && !isPrevRest)
            mapMix.insert(getCueID(score, startTick, score->lastSegment()->tick().ticks()), iName);
    }


    // Open a stream into the file
    QFile fileCDATA;
    fileCDATA.setFileName(QString("%1/%2%3%4%5").arg(qfi->path())
                                              .arg(score->metaTag(tagWorkNo))
                                              .arg(SMAWS_)
                                              .arg(score->metaTag(tagMoveNo))
                                              .arg(EXT_VTT));
    fileCDATA.open(QIODevice::WriteOnly | QIODevice::Text);  // TODO: check for failure here!!!
    QTextStream streamCDATA(&fileCDATA);

    // Stream the header
    streamCDATA << SVG_CDATA_BEGIN;
                                   ...
                                   ++cue_id)
    {
        ...
        if (cue_id != keys.cbegin())  // no leading or trailing whitespace,
            streamCDATA << SVG_SPACE; // but space-delimited.
        streamCDATA << *cue_id << SVG_COMMA << values.join(SVG_COMMA);
    }
    streamCDATA << SVG_CDATA_END;
...
bool MuseScore::saveSMAWS_Lyrics(Score* score, QFileInfo* qfi)
{
            ...
            for (Segment* s = m->first(SegmentType::ChordRest); s; s = s->next(SegmentType::ChordRest))
            {
                ...
                if (isChord) {
                    ...
                    if (!isPrevRest && tick != 0) {
                        ...
                        mapVTT.insert(getCueID(score, startTick, tick), lyricsVTT);
    ...
    if (!isPrevRest) {
        lyricsVTT += VTT_CLASS_END;
        mapVTT.insert(getCueID(score, 
                               startTick, 
                               score->lastSegment()->tick().ticks()), 
                      lyricsVTT);
    ...
    qts << "<!-- " << score->metaTag("workTitle") << SVG_SPACE 
                   << smawsDesc(score)            << " -->" << endl;
    ...
    for (auto t = setVTT.begin(); t != setVTT.end(); ++t) {
        ...
        for (auto i = values.begin(); i != values.end(); ++i) {
            qts << "<span"
                << formatInt(SVG_CUE_NQ, tickToMsec(score, *t), CUE_ID_FIELD_WIDTH, true)
