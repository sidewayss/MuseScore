...
void SvgPaintEngine::drawTextItem(const QPointF &p, const QTextItem &textItem)
{
    ...
    if (_isSMAWS && !_isMulti && _et == EType::TEXT) {
        switch(Ms::SubStyleId(static_cast<const Ms::Text*>(_e)->subtype())) {
        case Ms::SubStyleId::TITLE:
        case Ms::SubStyleId::SUBTITLE :
            x = ((_sysRight - _sysLeft) / 2) + _sysLeft; // centered
            break;
        case Ms::SubStyleId::COMPOSER:
            x = _sysLeft;
            break;
        case Ms::SubStyleId::POET:
            x = _sysRight;
            break;
        default:
            x = p.x();
